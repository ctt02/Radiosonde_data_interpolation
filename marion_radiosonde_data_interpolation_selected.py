###This python script is extension to <marion_radiosonde_data_interpolation.py>.
...
###Few selected Altitude values data will be written separately to the interpolated one.
...
selected_altitudes = [50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000]
...
for input_file in csv_files:
    ...
    selected_data = pd.DataFrame(columns=output_data.columns)
    ...
    selected_rows = output_data[output_data['Height (m)'].isin(selected_altitudes)]
    selected_data = pd.concat([selected_data, selected_rows], ignore_index=True)

    selected_file = os.path.join(folder_path, file_name_without_ext + "_selected.csv")
...
#    print(selected_data)
    print("Selected data saved to", selected_file)
